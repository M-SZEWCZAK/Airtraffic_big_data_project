...
airport_bronze_path = "hdfs:///original_data/airports"  # Assuming airports are in original_data
...
def get_bronze_files():
    ...
    bronze_files = {}
    pattern = re.compile(r'(\d{4})_(\d{1,2}).*\.parquet')
...
def get_existing_partitions():
    ...
    try:
        ...
        for row in existing_partitions:
            ...
            mon = int(parts[1].split('=')[1])
            partitions.add((yr, mon))
...
def load_airport_data():
    """
    Load airport dimension data from bronze layer
    """
    # Try to find airport file
    result = subprocess.run(
...
def process_partitions(year_month_list=None, force_reprocess=False, max_distance_km=50):
    """
    ...
    for yr, mon in sorted(partitions_to_process):
        if (yr, mon) not in bronze_files:
            print(f"Warning: No bronze file found for year={yr}, month={mon}")
            continue

        file_path = bronze_files[(yr, mon)]
        print(f"\n{'=' * 80}")
        print(f"Processing: {file_path}")
        print(f"Partition: year={yr}, month={mon}")
        print(f"{'=' * 80}")
        ...
        try:
            ...
            result = result.withColumn("month_partition", lit(mon))

            # Write partition
            partition_output = f"{silver_path}/year_partition={yr}/month_partition={mon}"
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--full":
            ...
            for i in range(2, len(sys.argv)):
                yr, mon = sys.argv[i].split(',')
                partitions.append((int(yr), int(mon)))
            process_partitions(year_month_list=partitions, max_distance_km=MAX_DISTANCE_KM)
        else:
            print("Usage:")
